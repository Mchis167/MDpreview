...
4.  **Git Push Dependency (Bắt buộc)**:
    *   **LUÔN LUÔN** thực hiện cập nhật `CHANGELOG.md` **TRƯỚC KHI** thực hiện bất kỳ lệnh `git push` hoặc workflow `/04.github` nào.
    *   Changelog là nguồn sự thật (source of truth) cho mọi bản release.
...
> **Lưu ý**: Đây là bước bắt buộc phải hoàn tất trước khi chạy `/04.github`.