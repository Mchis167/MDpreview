...
> Lớp vỏ hợp nhất (Unified Glass Shell) cho tất cả floating menu trong ứng dụng — xử lý positioning, glassmorphism, và singleton lifecycle.
...
3. **`anchor`** — đặt dưới hoặc trên anchor button:
   - Nếu đủ chỗ phía dưới → hiển thị bên dưới
   - Nếu không đủ chỗ phía dưới nhưng đủ phía trên → lật lên trên
4. **Screen bounds check** — luôn clamp vào viewport với `--ds-space-md` padding

Các giá trị margin/padding đọc từ Design Tokens (`--ds-space-lg`, `--ds-space-md`) để đồng bộ với hệ thống spacing.
...
```js
ExplorerSettingsComponent.toggle({ anchor: buttonEl });
// → Kiểm tra MenuShield.active → gọi MenuShield.open({ anchor, content })
```

### Toggle thông minh
Khi người dùng click lại vào button kích hoạt trong lúc menu đang mở, `anchor` trong outside-click listener bị bỏ qua — component gọi (ví dụ: `ExplorerSettingsComponent.toggle()`) tự xử lý logic toggle thông qua `MenuShield.active`.
