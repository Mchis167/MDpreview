...
- **Wiki Backlinks Drawer (Phase 5)**: Triển khai hệ thống xem các liên kết ngược (Backlinks) thông qua giao diện side-drawer hiện đại.
    - **`BacklinksDrawerComponent` (Organism)**: Quản lý danh sách backlinks với khả năng xem nhanh tài liệu liên quan mà không mất ngữ cảnh.
    - **Waypoints Icon**: Đăng ký biểu tượng `waypoints` mới vào Design System cho nút điều hướng backlinks.
    - **Empty State Illustration**: Thiết kế giao diện trống chuyên nghiệp (với icon và hướng dẫn) khi tài liệu không có liên kết ngược.
- **Resizable Side Drawer**: Nâng cấp `WikiDrawer` hỗ trợ thay đổi kích thước linh hoạt.
...
- **Side-by-Side Drawer Layout**: Tối ưu hóa giao diện khi mở đồng thời nhiều drawer.
    - **Auto-Shifting Logic**: `WikiDrawer` tự động dịch sang trái khi `BacklinksDrawer` được mở để tránh đè lên nhau.
    - **Coordinated Overlay**: Lớp phủ (Overlay) của các drawer tự động co lại để chỉ che phần nội dung chính, cho phép tương tác/cuộn đồng thời trên cả hai panel drawer.
